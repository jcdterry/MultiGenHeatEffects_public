...
Fly_data %>%
 rename ( `G0 Temperature` = G0) %>%
  ggplot() +
  geom_point(aes(G1, Count), alpha=0.5, size = 1) +
...
Fly_data %>%
 rename ( `G0 Temperature` = G0) %>%
  ggplot() +
  geom_boxplot(aes(G1, Count, group = G1))+ 
  geom_point(aes(G1, Count), alpha=0.5, size = 0.5) +
  geom_line(aes(G1, Expectation), col = '#0072B2',
            data =  rename (Prediction,`G0 Temperature` = G0) ) +
  theme_bw(base_size = 12) +
  theme(text = element_text(size = 10),
...
ggsave('Figures/Fly TPC Fit_boxplot.png', width = 6, height = 4 )
